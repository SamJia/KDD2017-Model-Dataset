...
void Model::Load(int argc, char const *argv[]) {
	input_folder_path_ = GetArgByTag(argc, argv, "-i", string("."), "Input data's folder path");
	dataset_.set_in_dir(input_folder_path_);
	dataset_.LoadData();
	dataset_.NormalizeTime();
	output_file_perfix_ = GetArgByTag(argc, argv, "-o", string("."), "Output Graph data prefix");
	...
	iteration_count_ = GetArgByTag(argc, argv, "-ic", 200, "Number of update iterations");
	save_iteration_count_ = GetArgByTag(argc, argv, "-sic", 50, "How many iterations for once save");
	...
	double default_lambda1 = lambda_weight * log(
	                             ...
	                             / max(community_count_ * community_count_, 1));
	...
	lambda_.second = default_lambda1;
	if (dataset_.user_count() == 0){
